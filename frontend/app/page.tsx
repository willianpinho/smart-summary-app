...
export default function Home() {
  ...
  const handleSummarize = async (text: string) => {
    ...
    try {
      ...
      // Carries bytes/text that don't yet form a complete "\n\n"-delimited
      // event across reads -- a chunk boundary can land mid-event, and a
      // multi-byte UTF-8 character can land mid-byte-sequence.
      let buffer = "";
      ...
      while (true) {
        ...
        // { stream: true } keeps a trailing partial multi-byte UTF-8
        // sequence in the decoder's internal state instead of emitting a
        // replacement character, so accents/emoji split across reads
        // decode correctly once the rest of the sequence arrives.
        buffer += decoder.decode(value, { stream: true });

        // SSE format: events are separated by \n\n. Keep the last,
        // possibly-incomplete segment in the buffer for the next read.
